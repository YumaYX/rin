...
async fn main() -> Result<()> {
    ...
    let conn = Arc::new(Mutex::new(Connection::open("urls.db")?));
    {
    ...
    // list.txt を読み込む
    let urls: Vec<String> = fs::read_to_string("list.txt")
            ...
            let title = match reqwest::get(&url).await {
                Ok(resp) => match resp.text().await {
                    Ok(text) => {
                        ...
                        document.select(&selector).next().map(|e| e.text().collect::<String>())
                    }
                    Err(_) => None,
                },
                Err(_) => None,
